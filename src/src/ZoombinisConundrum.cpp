...
int main()
{
	...
	for (int q = 0; q < 16; q++) {
		srand(time(0));
		groupman[q].identity = (rand() % 625);
		for (int r = 0; r < q; r++) {
			if (groupman[q].identity == groupman[r].identity) { //preventing repeat zoombinis
				groupman[q].identity = (rand() % 625);
			}
		}
		s = groupman[q].identity;
		

		std::cout << "Member " << (q+1) << ": Zoombini " << groupman[q].identity << " (Features: " << zoombinis[s].eyes << ", " << zoombinis[s].hair << ", " << zoombinis[s].nose << ", " << zoombinis[s].loco << ") \n";
